...
public class extensionHangman extends ConsoleProgram {
	/** Initializing variable for totall attempts */
	private static final int NTURNS = 8;
	
	// Initializing pannel Width and Height.
	...
	private extensionHangmanCanvas canvas;
	private extensionHangmanLexicon lexicon;
	...
	public void init() {
		canvas = new extensionHangmanCanvas(); // initializing canvas class
	...
	public void run() {
		...
		lexicon = new extensionHangmanLexicon(); // initializing lexicon class
	...
	private void outputText() {
		...
		if (lettersGuessed.contains(Character.toString(yourGuess))) {
			...
			// output streak if it's 2 or more in a row.
			if (streak > 1) {
				canvas.drawStreak(streak);
			}
