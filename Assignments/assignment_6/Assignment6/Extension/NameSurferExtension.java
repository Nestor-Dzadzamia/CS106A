...
/* initializing instance variables */
	...
	private NameSurferDataBaseExtension data = new NameSurferDataBaseExtension("names-data.txt");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == graph || e.getSource() == field) {
			...
			if (data.findEntry(field.getText()) != null) { // first checking if Entered name is in Database or not.
				...
				NameSurferEntryExtension entry = data.findEntry(field.getText()); // for Following name input we create Entry class
			...
			canvas.delete(deleteField.getText());
			canvas.update();
			deleteField.setText("");
		}
	}
